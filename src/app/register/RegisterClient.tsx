  ...
  const handleGoogleSignUp = async () => {
    ...
    try {
      ...
      if (gErr.code !== 'auth/popup-closed-by-user') {
        setError(gErr.message || 'Google sign-up failed. Please try again.');
      }
    } finally {
      setLoading(false);
    }
    ...
    <main style={{
      ...
      <div className={`glass-panel fade-in ${shake ? 'shake' : ''}`} style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '1.5rem' }}>
          ...
          <span style={{ fontSize: '0.75rem', color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.05em' }}>OR EMAIL SIGN UP</span>
          <div style={{ flex: 1, height: '1px', background: 'var(--border-color)' }} />
        </div>
